...
const hoodToggle = document.getElementById("hoodToggle");
    ...
    loader.load(
        ...
        (gltf)=>{
         ...
         if (gltf.animations.length > 0) {
           hoodAction = mixer.clipAction(gltf.animations[0]);
           hoodAction.clampWhenFinished = true;
          hoodAction.loop = THREE.LoopOnce;
         }
    ...
    if(data.related){
        ...
        data.related.forEach(item=>{
            ...
            card.innerHTML=`
                ...
                <div class="related-price">${item.price} ريال</div>
            `;
...
document.getElementById("adminBtn").onclick = ()=>{
    ...
    if(!hoodAction) return;

    if(!hoodOpen){

        hoodAction.reset();
        hoodAction.timeScale = 1;
        hoodAction.play();

        hoodToggle.textContent = "🔒 إغلاق الكبوت";

        hoodOpen = true;

    }else{

        hoodAction.paused = false;
        hoodAction.timeScale = -1;
        hoodAction.play();

        hoodToggle.textContent = "🔧 فتح الكبوت";
